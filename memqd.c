...
void process_command(conn *c, char *command) {
    ...
    if (strncmp(command, "set ", 4) == 0 && (comm = NREAD_SET)) {
...
void usage(void) {
    ...
    printf("-i            print memqd and libevent license\n");
...
void usage_license(void) {
    printf(PACKAGE " " VERSION "\n\n");
    printf(
	"Copyright (c) 2008, Roast. <http://www.libing.name/>\n"
	"All rights reserved.\n"
	"\n"
    );
