...
typedef struct _qitem{
	...
	void * end[0];	/*- the queue item data -*/
} qitem;

/*- the queue name space item -*/
typedef struct _stritem {
    struct _stritem *next;
    struct _stritem *prev;
    struct _stritem *h_next;  /* hash chain next */

	qitem *head;	/*- the head item of the queue -*/
	qitem *tail;	/*- the tail item of the queue -*/
    ...
    void * end[0];				/*- fixed only store the queue name -*/
} item;
...
#define QITEM_data(qitem) ((char*) &((qitem)->end[0]))
